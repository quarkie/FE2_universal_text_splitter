...
public class IlsAaEmailParserTest {
  ...
  public void testParseEmailText() {
    String emailText =
        "SE2 / THL Türöffnunggedruckt am 22.Oktober 2024 9:29\n"
            + "\n"
            + "Einsatzanlass\n"
            + "\n"
    ...
    assertEquals(
        "FL Rosenberg 1/19-1|FL Rosenberg 1/43-1", result.get(EAlarmDataEntries.VEHICLES.getKey()));
    assertEquals("SE2", result.get(EAlarmDataEntries.KEYWORD.getKey()));
    assertEquals(
        "F SE2 Türe öffnen/verschliessen",
        result.get(EAlarmDataEntries.KEYWORD_DESCRIPTION.getKey()));
    // assertEquals("SE2", result.get(EAlarmDataEntries.KEYWORD_IDENTIFICATION.getKey()));
    assertEquals(
        "RD und Pol auf Anfahrt", result.get(EAlarmDataEntries.KEYWORD_ADDITIONAL.getKey()));
