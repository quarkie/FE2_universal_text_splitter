...
public class IlsAaEmailParser {
    ...
    private static final Pattern UNITS_PATTERN = Pattern.compile("(FF\\s+.+?)\\n(\\d{2}:\\d{2}:\\d{2})", Pattern.DOTALL); // Matches units followed by a timestamp
    private static final Pattern VEHICLES_PATTERN = Pattern.compile("FL\\s+[\\w\\s]+\\s+\\d+/\\d+-\\d+");
    private static final Pattern COORDINATES_PATTERN = Pattern.compile("WGS\\s+[NS]?(\\d+\\.\\d+)\\s+[EW]?(\\d+\\.\\d+)");
    ...
    public static Map<String, String> parseAlarmText(String text) {
        ...
        resultMap.put(EAlarmDataEntries.POSTALCODE.getKey(), Utility.extractPattern(text, POSTAL_CODE_PATTERN));
        resultMap.put(EAlarmDataEntries.BUILDING_NAME.getKey(), Utility.extractValue(text, "Ortszusatz"));
        resultMap.put(EAlarmDataEntries.LOCATION_ADDITIOnAL.getKey(), Utility.extractValue(text, "Ortsteil"));
        ...
        resultMap.put(EAlarmDataEntries.KEYWORD.getKey(), Utility.extractPattern(text, KEYWORD_PATTERN));
        resultMap.put(EAlarmDataEntries.KEYWORD_ADDITIONAL.getKey(), Utility.extractValue(text, "Meldebild"));
        resultMap.put(EAlarmDataEntries.KEYWORD_DESCRIPTION.getKey(), Utility.extractValue(text, "Stichwort"));
        resultMap.put(EAlarmDataEntries.KEYWORD_IDENTIFICATION.getKey(), Utility.extractValue(text, "TODO"));
        resultMap.put(EAlarmDataEntries.KEYWORD_ADDITIONAL.getKey(), Utility.extractValue(text, "Zusatz"));
    ...
    private static List<String> extractUnits(String text) {
        ...
        while (matcher.find()) {
            unitsList.add(matcher.group(1).replace("\n\n", " ").trim());
        }
        return unitsList;
    }
    ...
    private static List<String> extractVehicles(String text) {
        ...
        while (matcher.find()) {
            vehiclesList.add(matcher.group().trim());
        }
        return vehiclesList;
    }
    ...
    private static void addIncrementedParams(Map<String, String> resultMap, List<String> items, String prefix) {
        for (int i = 0; i < items.size(); i++) {
            resultMap.put(prefix + "_" + (i + 1), items.get(i));
        }
    }

    private static void addValueParams(Map<String, String> resultMap, List<String> items, String prefix) {
