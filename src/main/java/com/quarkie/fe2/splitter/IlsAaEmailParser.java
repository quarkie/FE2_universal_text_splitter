...
import java.util.regex.Pattern;
    ...
    public class IlsAaEmailParser {
        ...
        private static final Pattern UNITS_PATTERN = Pattern.compile(
                "(FF\\s+.+?)\\n(\\d{2}:\\d{2}:\\d{2})", // Matches units followed by a timestamp
                Pattern.DOTALL);
        private static final Pattern VEHICLES_PATTERN = Pattern.compile("FL\\s+[\\w\\s]+\\s+\\d+/\\d+-\\d+");
        private static final Pattern COORDINATES_PATTERN = Pattern.compile("WGS\\s+([N|S]\\d+\\.\\d+)\\s+([E|W]\\d+\\.\\d+)");
        ...
        public static Map<String, String> parseAlarmText(String text) {
            ...
            if (coordinates != null) {
                ...
                resultMap.put(EAlarmDataEntries.LAT.getKey(), coordParts[0]);
                resultMap.put(EAlarmDataEntries.LNG.getKey(), coordParts[1]);
            }
